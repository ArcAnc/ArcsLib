...
public class PRenderTypes
{
	public static class RenderPipelinesProvider
	{
		...
		private static final BindGroupLayout TRIANGLES_INSTANT_LAYOUT = BindGroupLayout.builder().
				...
				withUniform("DeformerValues", UniformType.TEXEL_BUFFER, GpuFormat.RGBA32_FLOAT).
				build();
		private static final BindGroupLayout OIT_COMPOSITE_LAYOUT = BindGroupLayout.builder().
				withSampler("AccumSampler").
				withSampler("RevealSampler").
				withSampler("DepthSampler").
				build();
		private static final BindGroupLayout OIT_LAYER_LAYOUT = BindGroupLayout.builder().
				withSampler("LayerDepthSampler").
				build();
		private static final BlendFunction OIT_ACCUMULATION_BLEND =
				new BlendFunction(BlendFactor.ONE, BlendFactor.ONE, BlendOp.ADD);
		...
		public static final RenderPipeline TRIANGLES_OIT = registerPipeline(RenderPipeline.builder(TRIANGLES_SNIPPET).
				...
				withBindGroupLayout(BindGroupLayouts.SAMPLER1).
				withBindGroupLayout(OIT_LAYER_LAYOUT).
				withShaderDefine("ALPHA_CUTOUT", 0.1F).
				withCull(false).
				withColorTargetState(0, oitTarget(OIT_ACCUMULATION_BLEND, GpuFormat.RGBA16_FLOAT, ColorTargetState.WRITE_ALL)).
				withColorTargetState(1, oitTarget(OIT_ACCUMULATION_BLEND, GpuFormat.R16_FLOAT, ColorTargetState.WRITE_RED)).
				withDepthStencilState(DEPTH_TEST_NO_WRITE).
				build());
		...
		public static final RenderPipeline OIT_COMPOSITE = registerPipeline(RenderPipeline.builder().
				...
				withBindGroupLayout(OIT_COMPOSITE_LAYOUT).
				withPrimitiveTopology(PrimitiveTopology.TRIANGLES).
				withColorTargetState(new ColorTargetState(BlendFunction.TRANSLUCENT)).
		...
		private static ColorTargetState oitTarget(BlendFunction blend, GpuFormat format, int writeMask)
		{
			return new ColorTargetState(Optional.of(blend), format, writeMask);
		}
