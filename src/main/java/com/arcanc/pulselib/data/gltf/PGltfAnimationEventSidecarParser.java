...
public class PGltfAnimationEventSidecarParser
{
	...
	{
		ResourceLocation sound = ResourceLocation(stringValue(member(node, "sound"), stringValue(member(node, "id"), "")));
		if (sound == null)
			return null;
	...
	{
		ResourceLocation particle = ResourceLocation(stringValue(member(node, "particle"), stringValue(member(node, "id"), "")));
		if (particle == null)
			return null;
	...
	private static @Nullable ResourceLocation ResourceLocation(String value)
	{
		if (value.isBlank())
			return null;
		...
		if (id == null)
			PLibDatabase.LOGGER.warn("Invalid GLTF animation sidecar event ResourceLocation: {}", value);
		return id;
	}
