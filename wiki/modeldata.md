[`PModelData`](https://github.com/ArcAnc/PulseLib/blob/26.1/src/main/java/com/arcanc/pulselib/content/renderer/modelData/PModelData.java) is the small object that tells a renderer, "this is the model file, and these are the textures the model should use."
...
`PModelData.getTextureByName(name)` first checks explicit mappings added with `addTexture`. If a texture was not registered explicitly, PulseLib asks [`PModelCache.resolveTextureLocation`](https://github.com/ArcAnc/PulseLib/blob/26.1/src/main/java/com/arcanc/pulselib/util/PModelCache.java) to infer a path from the model path and texture name.
